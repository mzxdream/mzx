...
public:
    ...
    using PrintHandler = std::function<void(Level, const char *, int,
...
public:
    ...
    static void Print(Level level, const char *file_name, int line,
                      const char *func_name, Args &&... args)
    {
...
#define MZX_LOG(level, ...)                                                    \
    mzx::Logger::Print(level, __FILE__, __LINE__, __func__, ##__VA_ARGS__)
#define MZX_LOG_IF(level, condition, ...)                                      \
    !(condition) ? (void)0 : MZX_LOG(level, ##__VA_ARGS__)

#define MZX_DEBUG(...) MZX_LOG(mzx::Logger::Level::Debug, ##__VA_ARGS__)
#define MZX_DEBUG_IF(condition, ...)                                           \
    MZX_LOG_IF(mzx::Logger::Level::Debug, condition, ##__VA_ARGS__)

#define MZX_INFO(...) MZX_LOG(mzx::Logger::Level::Info, ##__VA_ARGS__)
#define MZX_INFO_IF(condition, ...)                                            \
    MZX_LOG_IF(mzx::Logger::Level::Info, condition, ##__VA_ARGS__)

#define MZX_WARN(...) MZX_LOG(mzx::Logger::Level::Warn, ##__VA_ARGS__)
#define MZX_WARN_IF(condition, ...)                                            \
    MZX_LOG_IF(mzx::Logger::Level::Warn, condition, ##__VA_ARGS__)

#define MZX_ERR(...) MZX_LOG(mzx::Logger::Level::Error, ##__VA_ARGS__)
#define MZX_ERR_IF(condition, ...)                                             \
    MZX_LOG_IF(mzx::Logger::Level::Error, condition, ##__VA_ARGS__)

#define MZX_FATAL(...) MZX_LOG(mzx::Logger::Level::Fatal, ##__VA_ARGS__)
#define MZX_FATAL_IF(condition, ...)                                           \
    MZX_LOG_IF(mzx::Logger::Level::Fatal, condition, ##__VA_ARGS__)

#define MZX_CHECK(EXPRESSION, ...)                                             \
    MZX_LOG_IF(mzx::Logger::Level::Fatal, !(EXPRESSION),                       \
               "CHECK failed:" #EXPRESSION " ", ##__VA_ARGS__)

#define MZX_CHECK_STATIC(...) static_assert(__VA_ARGS__)
