...
static void LogPrint(Logger::Level level, const char *file_name, int line,
                     const char *func_name, const std::string &message)
{
    const static char *level_names[] = {"DEBUG", "INFO ", "WARN ", "ERROR",
                                        "FATAL"};
    const static char *level_colors[] = {"\e[32m", "\e[1;37m", "\e[1;33m",
                                         "\e[31m", "\e[31m"};
    fprintf(stderr, "%s[mzx %s %s:%d %s] %s\n\e[0m",
            level_colors[EnumToValue(level)], level_names[EnumToValue(level)],
